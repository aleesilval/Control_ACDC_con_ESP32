...
<head>
  ...
  <style>
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .zcd-panel { background: #020617; border: 1px solid #1e293b; padding: 10px 15px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; gap: 10px; width: 100%; box-shadow: inset 0 2px 4px rgba(0,0,0,0.5); }
    .zcd-info-group { display: flex; align-items: center; gap: 8px; }
    ...
    .zcd-text { font-size: 10px; color: #94a3b8; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; }
    .zcd-val { color: #e2e8f0; }
    .btn-zcd-recalib { background: #1e293b; border: 1px solid #475569; color: #38bdf8; font-size: 10px; padding: 5px 8px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: all 0.2s; text-transform: uppercase; }
    .btn-zcd-recalib:active { background: #38bdf8; color: #0f172a; }
    ...
    .btn-lock { width: 100%; padding: 10px; border-radius: 8px; font-weight: bold; border: 1px solid #334155; cursor: pointer; transition: all 0.2s; margin-top: 5px; font-size: 12px; text-transform: uppercase; }
    .pot-unlocked { background: #1e293b; color: #10b981; border-color: #10b981; }
    .pot-locked { background: #7f1d1d; color: #ef4444; border-color: #ef4444; }
  ...
  <script>
    ...
    function toggleInvertirCanales() {
      invertirCanales = !invertirCanales;
      const btn = document.getElementById('invertChBtn');
      btn.innerText = invertirCanales ? '🔄 CANALES INVERTIDOS' : '🔃 INVERTIR CANALES';
      btn.className = invertirCanales ? 'btn-lock pot-locked' : 'btn-lock pot-unlocked';
    ...
    function sendP5Config() {
      ...
      const desfase = document.getElementById('cfgDesfase').value;
      const rampaP7 = document.getElementById('cfgRampaP7').value;
      
      limiteSupHW = parseInt(limSup);
      document.getElementById('sliderAlfa').min = limInf;
      document.getElementById('sliderAlfa').max = limSup;
      
      fetch('/config_p5?lim_inf=' + limInf + '&lim_sup=' + limSup + '&rampa_on=' + rampaOn + '&rampa_off=' + rampaOff + '&pulso=' + pulso + '&desfase=' + desfase + '&rampa_p7=' + rampaP7);
    }
    ...
    window.onload = function() {
      fetch('/get_config')
        ...
        .then(data => {
          ...
          invBtn.className = invertirCanales ? 'btn-lock pot-locked' : 'btn-lock pot-unlocked';
    ...
    setInterval(() => {
      fetch('/get')
        ...
        .then(data => {
          ...
          document.getElementById('zcdState').innerText = zcdSync ? 'SINCRONIZADO' : 'RE-SINTONIZANDO';
...
<body>
  ...
  <div class="container">
    ...
    <div class="zcd-panel">
      <div class="zcd-info-group">
        <div class="zcd-dot" id="zcdDot"></div>
        <span class="zcd-text"><span id="zcdState" class="zcd-val">Cargando...</span></span>
      </div>
      <div class="zcd-info-group">
        <span class="zcd-text"><span id="zcdTime" class="zcd-val">--</span> µs</span>
        <span class="zcd-text" style="color:#334155;">|</span>
        <span class="zcd-text"><span id="zcdFreq" class="zcd-val">--</span> Hz</span>
      </div>
      <button class="btn-zcd-recalib" onclick="triggerZcdRecalib()">🔄 Sincronizar</button>
    </div>
  ...
  <div id="configModal" class="modal-overlay">
    <div class="modal-card">
      ...
      <div id="panel-hidden-p5">
        ...
        <div class="config-field" style="border-color:#38bdf8; background:#0f172a;">
          <label style="color:#38bdf8;">Compensación Fase ZCD (µs)<br><small style="font-size:9px; color:#94a3b8;">Valores negativos adelantan el disparo.</small></label>
          <input type="number" id="cfgDesfase" value="0">
        </div>
        <div class="config-field">
          <label>Rampa P7 Lazo Cerrado (ms)</label>
          <input type="number" id="cfgRampaP7" value="3">
        </div>
